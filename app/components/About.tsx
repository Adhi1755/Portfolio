...
const AboutMeContainer = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (skillsContainerRef.current && skillsHeadingRef.current && skillsItemsRef.current.length > 0) {
        // Set initial states for skills elements
        gsap.set(skillsHeadingRef.current, {
          y: 60,
          opacity: 0
        });

        gsap.set(skillsItemsRef.current, {
          y: 40,
          opacity: 0,
          scale: 0.8
        });

        // Create skills timeline with ScrollTrigger
        ...
        skillsTL.to(skillsItemsRef.current, {
          y: 0,
          opacity: 1,
          scale: 1,
          duration: 1.2,
